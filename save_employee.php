...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!empty($name) && !empty($email_prefix)) {
    ...
    if ($stmt->execute()) {
      echo "<script>alert('Employee added successfully!'); window.location.href='admin_dashboard.php';</script>";
    } else {
      echo "<script>alert('Error: Could not save employee'); window.location.href='admin_dashboard.php';</script>";
    }
    $stmt->close();
  } else {
